...
const PatientDetails = ({ open, onClose, patient }) => {
  ...
  const handleFormSubmit = async () => {
    try {
      await axios.put(`http://localhost:3000/api/hbms/addbptest/${patient.id}`, formData);
      // Handle success or show a notification
      console.log("success");
  
      // Generate PDF
      const pdfResponse = await axios.get(`http://localhost:3000/api/hbms/listbpreport/${patient.id}`, {
